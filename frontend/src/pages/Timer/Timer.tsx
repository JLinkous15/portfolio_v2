...
const timeParser = (n: number) => {
  ...
  const minutes = Math.floor((n / 1000 / 60) % 60).toString().padStart(2, '0')
  return `${minutes}:${seconds}`}

  const timerReducer = (state: TimerType.TimeState, action: TimerType.TimerReducerAction) => {
    switch (action.type) {
      case TimerType.TimerActionEnum.SET:
        if (action.value) {
          return {...state, duration: action.value, timer: timeParser(action.value)}
        }
        return state
...
export const Timer = () => {
  ...
  return (
    <Stack direction="column" alignItems="center" spacing={3}>
      ...
      {Ref?.current ?
             <Button variant="tactile" onClick={() => {
